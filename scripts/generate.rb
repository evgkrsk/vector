...
class Guide
  attr_reader :source, :sink, :event_from, :event_to
...
guide_sources.each do |source|
  guide_sinks.each do |sink|
    ...
    content = File.read("#{ROOT_DIR}/scripts/generate/templates/guides/guide.md.erb")
    renderer = ERB.new(content, nil, '-')
    content =
      begin
        renderer.result((Guide.new(source, sink)).get_binding)
      rescue Exception => e
        raise(
          <<~EOF
    ...
    notice =
      ...
      <!--
            ...
            To make changes please edit the template located at: ./scripts/templates/docs/guide.md.erb
      -->
      EOF
